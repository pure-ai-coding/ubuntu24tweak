...
| 跳转历史光标位置 | `Alt+←` / `Alt+→` | `workbench.action.navigateBack/Forward` | 后退/前进到上一个光标位置；改自默认的 `Ctrl+Alt+-` / `Ctrl+Shift+-` |
...
### 跳转历史光标位置（Alt+← / Alt+→）
`workbench.action.navigateBack` / `navigateForward`，即「后退 / 前进」，在光标曾经停留过的位置（含跨文件跳转）之间来回跳。
VS Code Linux 默认是别扭的 `Ctrl+Alt+-` / `Ctrl+Shift+-`，这里改到 IDEA/浏览器风格的 `Alt+←` / `Alt+→`。
用户键位优先级高于默认值，故无需解绑；Linux 下 `Alt+←/→` 在编辑器内本就无默认绑定。
...
## 系统冲突检查
已对照 `list-hotkeys/hotkeys.md`：`Alt+Shift+↑/↓`、`Ctrl+D`、`Ctrl+Alt+L`、`Alt+←/→` 均不与 GNOME
全局热键冲突（输入法切换在本机是 `Super+Space`，非 `Alt+Shift`；工作区切换用的是带 `Super`/`Ctrl` 的
组合键如 `Super+Alt+←`，并非纯 `Alt+←/→`）。
