...
- [x] 全局 ctrl+f1 截图，可直接贴在屏幕上，或选择复制到剪贴板，双击可以销毁，拖动可以移动；鼠标滚轮可缩放。此功能可参照windows下的snipaste。 @done(2026-06-27)
    [2026-06-26] 已修复截图叠层的输入锁死问题：截图模式改为 `key-focus + ESC` 退出，去掉容易残留的 stage grab，并把工具栏/手柄的层级显式抬到截图遮罩之上，避免按钮被全屏捕获层挡住。
    [2026-06-26] 防范措施：加入自动超时退出兜底，且销毁时同步清理超时与延迟定位回调，避免未来再出现“模式退不掉、只能注销”的情况。
    [2026-06-26] 进一步降风险：截图遮罩暂时限定在主屏，保留其它屏幕可直接操作；若主屏再发生异常，也能切到别的屏幕开终端处理进程。
    [2026-06-26] 继续修正：截图 UI 改为单一 modal 根容器，工具栏按钮与 ESC 进入同一输入树，避免按钮被全屏捕获层吞掉导致“只能退出、不能贴/复制/保存”。
    [2026-06-26] 再修正按钮点不中：根容器改为主屏局部坐标，遮罩/边框/手柄/工具栏统一使用局部显示坐标；工具栏强制最后置顶，并在捕获层/root 的点击处理中按按钮实际屏幕区域兜底路由到贴屏/复制/保存。
    [2026-06-26] 最终交互改为键盘提交：去掉贴屏/复制/保存工具栏，框选完成后提示 `T 贴屏幕 / C 复制 / S 保存 / Esc 取消`，规避鼠标按钮被 Shell 遮罩/输入区域吞掉的问题。
    [2026-06-26] 修复只有 Esc 有效的问题：键盘处理接入 root、capture 与 stage captured-event 三条路径，T/C/S 同时按 key symbol 和 unicode 判断；提交时显示“正在处理截图...”，失败会通知原因。
    [2026-06-26] 修复提交后卡在“正在处理截图...”：GNOME 46 的 `Shell.Screenshot.screenshot_area()` 需要传 `Gio.OutputStream`，不能传文件路径；提交时隐藏截图叠层后下一轮 idle 再截图，并在提交中忽略鼠标重新框选、增加 10 秒超时兜底。
    [2026-06-26] 修正贴图/保存/手柄：T 贴图按图片贴纸 3px 内边距反向偏移，让截图像素对齐选区左上角，不再额外 +40；S 保存优先写入系统图片目录(无则 `~/Pictures`)并确保目录存在；拖动/缩放过程改由 stage captured-event 接管 motion/release，避免八向手柄松开后仍跟随鼠标。
    [2026-06-26] 继续修正：T 贴图再向左上微调 1px；八向缩放不再依赖手柄 actor 命中，改为在捕获层按坐标检测手柄区域(命中半径 10px)进入 RESIZING，并使用 Mutter `Meta.Cursor.*_RESIZE` 更新鼠标形状。
    [2026-06-27] 修正八向缩放释放丢失：拖拽期间短暂 `global.stage.grab(root)`，释放时 dismiss；若 BUTTON_RELEASE 仍丢失，则下一次 BUTTON_PRESS 先结束当前 SELECTING/MOVING/RESIZING，避免状态残留后重新框选。
    [2026-06-27] 修复八向调整后 T 贴图坐标/尺寸错误：选区统一走 `_normalizeSelection()`，提交截图、显示、命中使用同一标准矩形；RESIZING 改为 left/top/right/bottom 边界模型，只移动被拖动的边并限制在主屏 bounds 内。
    [2026-06-27] 完成遗留两项：截图遮罩/框选 bounds 从主屏改为所有显示器的联合区域；T 贴屏幕时截图贴纸按选区逻辑宽高显示，不再走普通剪贴板图片的 600×700 初始缩放限制，避免框选/调整后贴图变小。
